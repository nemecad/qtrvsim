...
void TLB::on_csr_write(size_t internal_id, RegisterValue val) {
    ...
    if (old_mode != new_mode) {
        flush_all_entries();
        DEBUG("TLB: SATP changed -> translation mode changed (%u -> %u), flushed all",
...
void TLB::flush_single(VirtualAddress va, uint16_t asid) {
    ...
    for (size_t w = 0; w < associativity_; w++) {
        ...
        if (e.valid && e.vpn == vpn && (e.asid == asid || asid == 0)) {
            ...
            e.valid = false;
            DEBUG("TLB[%s]: flushed VA=0x%llx ASID=%u (wildcard=%s)", tag,
    ...
    for (size_t s = 0; s < num_sets_; ++s) {
        for (size_t w = 0; w < associativity_; ++w) {
            ...
            if (e.valid && e.asid == asid) {
                ...
                uint64_t old_vpn = e.vpn;
                e.valid = false;
                emit tlb_update(static_cast<unsigned>(w), static_cast<unsigned>(s), false,
                                old_asid, old_vpn, 0ull, false, false, false, false, false, false, false);
                any_invalidated = true;
            }
        }
    }
...
TLB::TranslationResult TLB::translate_virtual_to_physical(AddressWithMode vaddr) {
    ...
    if (!should_translate) {
        return { vaddr, std::numeric_limits<size_t>::max() };
    }
    ...
    for (size_t w = 0; w < associativity_; w++) {
        ...
        if (e.valid && e.vpn == vpn && e.asid == asid) {
            if (!check_permissions(e, current_sstatus_raw, mode.priv(), mode.opkind())) {
                throw SIMULATOR_EXCEPTION(PageFault,  "TLB: access fault on TLB hit", "");
            }
    ...
    DEBUG("TLB[%s]: cached VA=0x%llx -> PA=0x%llx (ASID=%u) on miss", tag, (unsigned long long)virt,
...
WriteResult TLB::translate_and_write(AddressWithMode dst, const void *src, size_t sz, WriteOptions opts) {
    ...
    while (remaining > 0) {
        ...
        bool satp_mode_on = is_mode_enabled_in_satp(current_satp_raw);
        if (vm_enabled && satp_mode_on && (dst.access_mode().priv() != CSR::PrivilegeLevel::MACHINE)) {
            if (tr.entry && ensure_ad_bits(*tr.entry, AccessOp::WRITE) == UpdateStatus::UPDATE_REQUIRED) {
                DEBUG("TLB: Updated A/D bits in memory for PPN 0x%lu", tr.entry->phys.get_raw() >> 12);
            }
        }
...
ReadResult TLB::translate_and_read(void *dst, AddressWithMode src, size_t sz, ReadOptions opts) {
    ...
    while (remaining > 0) {
        ...
        bool satp_mode_on = is_mode_enabled_in_satp(current_satp_raw);
        if (vm_enabled && satp_mode_on && (src.access_mode().priv() != CSR::PrivilegeLevel::MACHINE)) {
            if (tr.entry && ensure_ad_bits(*tr.entry, AccessOp::READ) == UpdateStatus::UPDATE_REQUIRED) {
...
UpdateStatus TLB::ensure_ad_bits_impl(Entry &e, AccessOp op) {
    ...
    if (updated != raw) {
        pt_walk_mem->write(e.pte_addr, &updated, sizeof(RawPte), { .type = AccessEffects::REGULAR });
        e.A = true;
...
UpdateStatus TLB::ensure_ad_bits(Entry &e, AccessOp op) {
    return e.pte_bytes == 4
        ? ensure_ad_bits_impl<uint32_t>(e, op)
        : ensure_ad_bits_impl<uint64_t>(e, op);
}

template bool check_permissions<TLB::Entry>(const TLB::Entry &pte, uint64_t raw_sstatus, CSR::PrivilegeLevel priv, AccessOp op);
