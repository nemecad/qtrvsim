...
public:
    struct Entry {
        ...
        [[nodiscard]] bool w() const {
            return W;
        }

        [[nodiscard]] bool x() const {
            return X;
        }
    ...
    uint64_t root_page_table_ppn() const {
        switch (xlen) {
            case Xlen::_32:
                return current_satp_raw & ((uint64_t(1) << Sv32Pte::PPN_BITS) - 1ULL);
            case Xlen::_64:
                return current_satp_raw & ((uint64_t(1) << Sv39Pte::PPN_BITS) - 1ULL);
            default:
                return current_satp_raw & ((uint64_t(1) << Sv32Pte::PPN_BITS) - 1ULL);
        }
    }
