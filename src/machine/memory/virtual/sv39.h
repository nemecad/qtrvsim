...
struct Sv39Pte : public GenericPte {
    ...
    static constexpr unsigned VPN2_SHIFT = PAGE_SHIFT + VPN_BITS * 2; // VPN2 is the next field above VPN1.

    static constexpr unsigned VPN_MASK = (1u << VPN_BITS) - 1; // Mask to extract a single VPN level (9
    // bits, value 0..511).

    // Number of bits available for the physical page number (PPN) in SV39 PTE.
    // SV39 encodes a 44-bit PPN in a 64-bit PTE (PPN[0]=9 bits, PPN[1]=9 bits, PPN[2]=26 bits), which
    // permits addressing a large physical memory range.
