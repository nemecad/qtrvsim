...
namespace machine {
    ...
    struct WalkResult {
        ...
        WalkResult(Address phys_, std::unique_ptr<GenericPte> leaf_pte_)
            : phys(phys_), leaf_pte(std::move(leaf_pte_)) {}
    };
    ...
    public:
        explicit PageTableWalker(FrontendMemory *mem,
            std::function<std::unique_ptr<GenericPte>(uint64_t)> pte_factory = sv32_pte_factory());

        template<typename T, int max_level_idx>
        WalkResult walk(const VirtualAddress &va, uint64_t raw_satp, uint64_t raw_sstatus, const AccessMode &access_mode);
