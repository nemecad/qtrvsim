...
bool Core::handle_exception(
    ...
    // Decide whether to deliver to supervisor:
    // If the trap originated while running in SUPERVISOR or UNPRIVILEGED, deliver to S-mode.
    // If it originated in MACHINE mode, synchronous exceptions must not be delegated back to S-mode
    // Interrupt delegation (MIDELEG) is handled for asynchronous IRQs.
    bool deliver_to_s = false;
    if (origin_priv == CSR::PrivilegeLevel::SUPERVISOR
        || origin_priv == CSR::PrivilegeLevel::UNPRIVILEGED) {
        deliver_to_s = true;
    } else {
        if (excause == EXCAUSE_INT) {
            uint64_t mie = control_state->read_internal(CSR::Id::MIE).as_u64();
            uint64_t mip = control_state->read_internal(CSR::Id::MIP).as_u64();
            uint64_t irqs = (mie & mip) & 0xffffffffULL;
            if (irqs != 0) {
                auto lsb_index = [](uint64_t v) -> unsigned {
                    return static_cast<unsigned>(__builtin_ctzll(v));
                };
                unsigned irq_to_signal = lsb_index(irqs);
                uint64_t mideleg = control_state->read_internal(CSR::Id::MIDELEG).as_u64();
                if (((mideleg >> irq_to_signal) & 1ULL) != 0) {
                    deliver_to_s = true;
                } else {
                    deliver_to_s = false;
                }
    ...
    if (control_state != nullptr) {
        ...
        switch (excause) {
            ...
            case EXCAUSE_STORE_FAULT:
                ...
                break;
            default:
                control_state->write_internal(tval_id, 0);
                break;
        }
        ...
        if (control_state->read_internal(tvec_id) != 0 && !get_step_over_exception(excause)) {
            control_state->exception_initiate(
                get_current_privilege(), target_priv);
...
FetchState Core::fetch(PCInterstage pc, bool skip_break) {
    ...
    const AddressWithMode inst_addr = AddressWithMode(regs->read_pc(), make_access_mode(state, AccessOp::FETCH));
    ...
    if ((flags & IMF_EXCEPTION) && (excause == EXCAUSE_NONE)) {
        if (flags & IMF_EBREAK) {
            ...
            switch (get_current_privilege()) {
                case CSR::PrivilegeLevel::UNPRIVILEGED:
                    excause = EXCAUSE_ECALL_U;
                    break;
                case CSR::PrivilegeLevel::SUPERVISOR:
                    excause = EXCAUSE_ECALL_S;
                    break;
...
MemoryState Core::memory(const ExecuteInterstage &dt) {
    ...
    if (excause == EXCAUSE_NONE) {
        try {
            ...
            if (memread) {
                excause = EXCAUSE_LOAD_PAGE_FAULT;
            } else {
        ...
        if (dt.xret) {
            ...
            if (this->xlen == Xlen::_32)
                computed_next_inst_addr =
                    Address(control_state->read_internal(epc_reg).as_u32());
            else
                computed_next_inst_addr =
                    Address(control_state->read_internal(epc_reg).as_u64());
            csr_written = true;
        }
    }
